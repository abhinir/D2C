...
class ILQR:
    ...
    def linearize_dynamics(self, x, u, t):
        ...
        for i in range(self.n_state):
            ...
            e[i] = h
            A[:, i] = (self.dynamics(x+e, u, t) - self.dynamics(x-e, u, t))/(2*h)
        ...
        for j in range(self.n_control):
            ...
            e[j] = h
            B[:, j] = (self.dynamics(x, u+e, t) - self.dynamics(x, u-e, t))/(2*h)
    ...
    def quadratize_cost(self, x, u, index):
        ...
        for i in range(self.n_state):
            e = np.zeros((self.n_state))
            e[i] = h
            c_x[i] = (self.cost(x + e, u) - self.cost(x - e, u))/(2*h)
        
        for j in range(self.n_control):
            e = np.zeros((self.n_control))
            e[j] = h
            c_u[j] = (self.cost(x, u + e) - self.cost(x, u - e))/(2*h)
        ...
        for a in range(self.n_state):
            for b in range(self.n_state):
                ...
                e2 = np.zeros((self.n_state))
                e2[b] = h
                c_xx[a, b] = self.cost(x+e1+e2, u) - self.cost(x+e1-e2, u) - self.cost(x-e1+e2, u) + self.cost(x-e1-e2, u)
                c_xx[a, b] = c_xx[a, b]/(4*h*h)
        ...
        for a in range(self.n_control):
            for b in range(self.n_control):
                ...
                e2[b] = h
                c_uu[a, b] = self.cost(x, u+e1+e2) - self.cost(x, u+e1-e2) - self.cost(x, u-e1+e2) + self.cost(x, u-e1-e2)
        ...
        for a in range(self.n_control):
            for b in range(self.n_state):
                ...
                e2 = np.zeros((self.n_state))
                e2[b] = h
                c_ux[a, b] = self.cost(x+e2, u+e1) - self.cost(x-e2, u+e1) - self.cost(x+e2, u-e1) + self.cost(x-e2, u-e1)
    ...
    def forward_pass(self, x0, k, K, x, u):
        ...
        for t in range(self.horizon):
            ...
            trajectory_cost += self.cost(x_trajectory[:, t], u_trajectory[:, t])
    ...
    def backward_pass(self, x_trajectory, u_trajectory):
        ...
        V_x[:, -1], V_xx[:, :, -1] = self.quadratize_cost(x_trajectory[:, -1].flatten(), index=self.horizon, u=None)
        
        for i in range(self.horizon):
            A_t[:, :, i], B_t[:, :, i] = self.linearize_dynamics(x_trajectory[:, i], u_trajectory[:, i], i)
    ...
    def main_func(self, u_init, x0):
        ...
        x_trajectory, u_trajectory, cost = self.forward_pass(x0, np.zeros((self.n_control, self.horizon)), np.zeros((self.n_control, self.n_state, self.horizon)), np.zeros((self.n_state, self.horizon+1)), np.zeros((self.n_control, self.horizon+1)))
        x_rollout_trajectory, u_rollout_trajectory = self.rollout_trajectories(x_trajectory, u_trajectory)
        costs = [cost]
        ...
        for iteration in range(self.max_iterations):
            ...
            while flag:  # Line search loop
                ...
                if cost_new <= cost:
                    flag = False
                    if (cost - cost_new)/cost < 1e-6:
                        return x_trajectory, u_trajectory, costs
